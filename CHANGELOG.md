...
- **产品官网**（official/，2026-09-14）：五页静态站 + 独立 CI，SEO 三件套与可访问性补齐。
- **web 对话框 DSH 形态对齐**（2026-09-19，DESIGN §13.L / ADR D43）：22px 全圆输入卡、14px 会话字号、发送钮点睛蓝、流式扫光、用户气泡与操作行重排、窄屏适配。

### Fixed — 两轮外部审计整改（2026-09-18 ~ 2026-09-19）

- **引擎八项**：审批写盘失败不再放行（fail-closed）；`length` 续采样纳入步数预算；用户文件工具与 checkpoint 原子写；路径硬边界补符号链接解析；附件投影缓存；LLM 错误文案脱敏；会话文件坏尾行受控修复；事件背压分级与进度门自愈。
- **桌面**：首启失败引导窗（sidecar 早退不再静默秒退）与 BrowserWindow 显式安全加固。
- **web**：渲染韧性（ErrorBoundary/麦克风释放/剪贴板定时器）、全量回放代际协调（防旧快照覆盖直播事件）、会话页销毁闸门、查询与补全竞态收敛。
- **第二轮 UI 七项**：裸 `/settings` 重定向、+ 菜单 Esc 关闭、命令面板弹层残留、沙箱下拉截断、搜索清除钮、侧边栏会话右键菜单（归档/删除内联两段式确认）、窄屏工具栏适配。
- **发布链路**：server 打包产物外置导入可解析性自校验（pi-ai/pino 入 bundle）。
