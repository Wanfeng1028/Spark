...
## 组件重叠/遮挡专项测试新增工单（2026-09-18）

### WO-079　窄屏下输入框工具栏按钮溢出/截断
- **优先级**：P1
- **模块**：apps/web
- **文件**：`apps/web/src/features/chat/Composer.tsx`（输入框工具栏）
- **问题描述**：窄屏（~375px）下输入框内"逐项确认"下拉、"立即/插话/排队"切换组、发送按钮溢出输入容器右边界；"插话"文字被截断不可见；textarea 占位符与按钮行重叠
- **修复方式**：窄屏断点（<480px）下"逐项确认"和模型选择器折叠为图标按钮，"立即/插话/排队"切换组换行或折叠为下拉
- **验收标准**：375px 宽度下输入框内所有控件不溢出，文字完整可读
- **预估工时**：0.5 天
- **关联**：WO-076（窄屏胶囊折叠）
...
- **优先级**：P1
- **模块**：apps/web
- **文件**：`apps/web/src/features/chat/Composer.tsx`（+ 附件菜单 Popover）
- **问题描述**：+ 附件菜单弹出后按 Escape 键，菜单不关闭；需再次点击 + 按钮才能切换关闭
- **修复方式**：为 + 附件菜单添加 Escape 键关闭事件，或统一使用带 dismissOnEscape 的 Popover 组件
- **验收标准**：菜单弹出时按 Escape 立即关闭
- **预估工时**：0.2 天

### WO-081　Ctrl+K 命令面板打开时 + 附件菜单未自动关闭
- **优先级**：P2
- **模块**：apps/web
- **文件**：`apps/web/src/features/chat/`（全局浮层状态管理）
- **问题描述**：+ 附件菜单打开时按 Ctrl+K 打开命令面板，底层 + 菜单仍然渲染（被遮罩覆盖但未关闭）；关闭 Ctrl+K 后 + 菜单仍然打开
- **修复方式**：打开全局命令面板时自动关闭所有局部浮层（+菜单、@补全、/命令面板）
- **验收标准**：Ctrl+K 打开时所有其他浮层均关闭；关闭后不再残留
- **预估工时**：0.3 天

### WO-082　设置页 bash 沙箱下拉选项文字截断
- **优先级**：P2
- **模块**：apps/web
- **文件**：`apps/web/src/pages/settings/General.tsx`（bash 沙箱下拉）
- **问题描述**：bash 沙箱下拉按钮显示"开启（平台 wrapper 隔..."，右侧被截断，完整文本为"开启（平台 wrapper 隔离）"
- **修复方式**：增加下拉按钮最小宽度，或文字截断时加 tooltip 显示完整内容
- **验收标准**：下拉按钮内文字完整显示，或悬停时 tooltip 展示全文
- **预估工时**：0.2 天
