...
3. **密度改造**：对齐 DESIGN.md §3 与 §13.B——13px 字号 / 28-32px 按钮高 / 8px 内边距 / **圆角只取 §13.B 封闭集档位**（工单 18.1 改判：控件为胶囊 full、卡片 12/16px；数字不在本文复制，规格唯一来源是 §13.B）/ mono 场景。
4. **黑名单自查**（DESIGN.md §7 + §12 AI 生成风清单）：无蓝紫渐变 / 无玻璃拟态与内衬模糊 / 无暖调配色 / 无超大字号 / 无 emoji 装饰 / 无 bento·三卡模板布局 / 无外部 CDN 资源 / 无整页滚动；提交前跑 §12.8 grep 清单（11 项模式：`rounded-2xl` 一项按 §13.B 档位判放行——工单 18.1 注记，其余零命中）。
5. **四态**：空 / 加载 / 错误 / 禁用。
