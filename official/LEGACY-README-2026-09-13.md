<!-- 冻结的历史快照（2026-09-13 注记）：本文件是 README 工单 11.8 重写之前导出的旧版本
（词表计数/状态徽章等停在当时时点，检查器 SKIP_DIRS 内不参与校验）。**现状唯一权威 = 仓库根 README.md**；
本文件保留供历史对照，不随现状更新。处置判决（重新导出 or 改薄壳）登记于 doc/02 v4.40/v4.50。 -->
...
![status](https://img.shields.io/badge/status-v1_五阶段完成-green)



![node](https://img.shields.io/badge/node-%E2%89%A5_22-3f3f46)



![react](https://img.shields.io/badge/react-19-3f3f46)



![ts](https://img.shields.io/badge/typescript-strict-3f3f46)



![monorepo](https://img.shields.io/badge/pnpm-monorepo-3f3f46)
...
\~/.spark/sessions/\<cwd>/\<ses\_id>.jsonl    durable 事件日志（append-only，可回放）
```
...
pnpm --filter web dev       # 仅前端（VITE\_SPARK\_MOCK=1 可脱离后端跑 Mock）

pnpm --filter cli dev       # CLI TUI（Ink，工单 8.2；需 server 在跑；--api \<url>/SPARK\_API 指基址，缺省 127.0.0.1:4318）
...
| v1.12 | 2026-08-24 | AI 编写：Trae・GLM-5.3；发起：晚风（Wanfeng1028，阶段三完成）                                                                                                | **当前状态与状态徽章更新为阶段三完成**（工单 11 pino 日志 + 脱敏 + examples/e2e-smoke.sh 验收脚本）；全仓 367 例（engine 251 + server 23 + web 47 + protocol 46）；doc/02 §8 v2.15 同步勾选 pino 行；用户自配 DEEPSEEK\_API\_KEY 后由 examples/e2e-smoke.sh 完成真实模型验收三场景                                                                                                                                                                                                                                                                                                                                 |
...
| v1.27 | 2026-09-05 | AI 编写：ZCode CLI · GLM-5.3-Flash（`builtin:zai-start-plan/GLM-5.3-Flash`）；排版改动产自工作区在途编辑（非本会话，来源会话不可考）；代提交发起：晚风（"push"指令） | 纯排版规范化：badge 逐个分行、列表符号与空行调整（- 改 *）、表格重对齐、全角标点统一（· → ・）、架构图与路径补转义字符及 `&#x20;` 空格实体（formatter 产物）；正文语义零变更；与根 README 的内容对账不在本行范围 |