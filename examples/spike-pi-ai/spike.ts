...
class Recorder {
  ...
  report(label: string): void {
    ...
    console.log(
      `text_delta: ${this.textDeltas.length} 次，长度分布：[${this.textDeltas.map((d) => d.len).join(',')}]`,
    )
    console.log(
...
async function main(): Promise<void> {
  ...
  console.log('\n########## 1. 流式回调（纯文本 + 诱导思考） ##########')
  {
    ...
    console.log(
      'textBuf 与定稿一致:',
      rec.textBuf ===
        (final.content.find((c) => c.type === 'text') as { text: string } | undefined)?.text,
    )
  }
  ...
  console.log('\n########## 2. 工具调用一轮（v2.7 薄桥验证） ##########')
  {
    ...
    const call = r1.final.content.find(
    ...
    console.log(
      ...
      r1.final.stopReason,
      '| toolCall:',
      call ? `${call.name}(${JSON.stringify(call.arguments)})` : '（无）',
    )
    ...
    const messages: Message[] = [...context.messages, r1.final, result]
    ...
    const text = r2.final.content.find(
  ...
  console.log('\n########## 3. 中途 abort（错误形态 + 已交付前缀） ##########')
  {
    ...
    const partialText = final?.content.find(
      (c): c is Extract<typeof c, { type: 'text' }> => c.type === 'text',
    )
