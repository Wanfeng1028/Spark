...
// 正则提取 version——JSON.parse 返回 any 会触发 no-unsafe-assignment（typed lint 对 .mjs 生效）
const pkgRaw = readFileSync(pkgPath, 'utf8')
const versionMatch = /"version"\s*:\s*"([^"]+)"/.exec(pkgRaw)
const cliVersion = versionMatch?.[1] ?? '0.0.0'
