/**
 * 配对入口解析（工单 9.4——语义对齐 apps/mobile/src/transport/pair-link.ts，
 * DESIGN §13.J.2.9：QR 内容 `spark://pair?host=<host>&port=<port>&code=<短码>`）。
 * 小程序端：Taro.scanCode 扫得字符串后走同一解析；扫不到/解析失败不阻塞，
 * 落回手输 6 位码 + 手输地址路径（设置页）。纯函数——深链与手输共用口径。
 */
...
/** 合法主机段：白名单（字母/数字/点/连字符）——黑名单会漏 `:` 与 `\` */
function isValidHost(host: string): boolean {
  return /^[A-Za-z0-9.-]+$/.test(host)
}

/** 仅匹配 `spark://pair?<query>` 形态（小程序环境无 URL 全局，手工解析） */
