/**
 * 错误码→人话文案表（工单 6.7 / D22 四端共享资产之一 / doc/07 H14）：
 * 单一来源——server §7.4 全部错误码 + transport/mock 特有码；四端（web/desktop/cli/mobile）
 * 直接复用本表，后续下沉位置随工单 8.1 一并评估（现阶段在 web 内单一来源即可）。
 * 约定：传输层错误统一形如 "E_CODE: 原始消息"（http.ts req / mock 同构）；
