...
3a. **本机禁止一切下载**（晚风 2026-09-12 拍板依赖包、2026-09-13 扩展为总则）：不在本机执行任何会发起下载的命令——`pnpm add` / `pnpm install` / `pnpm update`（依赖包）、`npm install -g` / `pip install` / `apt-get install` 等（外部工具）、以及浏览器/CDN 拉取。排查用命令全部依赖仓库既有 node_modules；走查所需的外部工具（SoX、语言服务器等）一律由 CI 在 runner 上安装并跑真实链路测试（测试用 `skipIf(环境无工具)` 条件化——CI 有则真跑、本地无则 skip 绿）。**新增依赖的流程**：AI 手改 package.json 声明（版本写明确区间）并在提交说明注明"锁文件待 install 生成"，`pnpm-lock.yaml` 的重算由人类在本机执行或明示授权后进行；CI 的 `--frozen-lockfile` 在锁文件未同步前会红在 install 步——属预期中间态，不得为绿灯而在本机偷跑 install。
...
# 质量闸（与 ci.yml 关卡同序：文档检查器 → typecheck → lint → knip → 生成物同步三件套（contract/events/openapi，gen + diff）→ test → eval → build）
...
pnpm typecheck                                # = pnpm -r typecheck（15 个 workspace package 含内部包；
