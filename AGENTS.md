...
| v1.42 | 2026-09-11 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，"工单要全部做完"指令） | §2.8 事实修正：事件词表 **22 → 26 种**（阶段十六工单 16.7 /goal 持续目标新增 goal.set/updated/completed/paused 四枚——全 durable 非 surface，目标内容经合成续跑 user.message 进模型历史；ADR D33 定三护栏数值：迭代上限 50 / token 预算 200k / judge 超时 25s；reducer 单测与引擎 goals.test 已补，走 new-event-type 全流程）。同批同步：doc/02 v4.42（§4.3/§4.4/§6.4 三表）、ARCHITECTURE v1.42（D33 + 事件模型行）、README v1.36、doc/08 v1.41（§16.7 进度）；两份生成物已重跑（契约用例与文档站词表页 26 种） |
| v1.43 | 2026-09-11 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（工单 16.9 /lsp LSP 集成完整落地指令） | §2.8 事实修正：事件词表 **26 → 27 种**（阶段十六工单 16.9 /lsp 新增 `lsp.diagnostics`——durable 非 surface，诊断经 lsp 工具进模型上下文即模型可见必被记录，空数组 = 诊断清零的真实发布；ADR D35 定换基座 vscode-languageserver-protocol + vscode-jsonrpc（MIT）与两必抄安全细节：spawn 前敏感 env 剥离、config hash 不变不重启；engine lsp/ 模块 + lsp 单工具 12 操作 + /lsp 命令基线 18→19 + Transport.listLspServers 三通道对等，走 new-event-type + new-tool 全流程）。同批同步：doc/02 v4.44（§4.3/§4.4/§6.4/§4.5/§4.7/§5.10 六处 + §8.7 V2-30 收口）、ARCHITECTURE v1.45（D35 + 事件模型行）、README v1.37、README.en、doc/03 v1.4、doc/08 v1.44（§16.9 进度）；两份生成物已重跑（契约用例 100 describe/966 断言与文档站词表页 27 种） |
| v1.44 | 2026-09-12 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，"工单要全部做完"指令） | **新增 D36 /agents 子代理管理（工单 16.2）**：两口子拍板（"全部做完"授权）——**格式维持 JSON 单一来源**（13.5 AgentPresetSchema 不变，不引入 MD+frontmatter；配置面全族统一），**两层定义**（项目层 .spark/agents/*.json 覆盖用户层同名档，DTO 增 source 合成值）；**启停放 spark.json agents.disabledAgents 名单**（PUT /api/settings 既有链路，重启档，SETTINGS_RESTART_REQUIRED 同步；停用档 task preset 解析即 E_CONFIG 拒绝，不静默回退）；命令基线 19→20（/agents client 命令，四包断言同改）。同批同步：doc/02 v4.45（§4.5 行 + §8.7 V2-33 收口）、ARCHITECTURE v1.46（D36）、README v1.38、doc/08 v1.45（§16.2 进度）。本批本机零验证 |
| v1.45 | 2026-09-12 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（"工单要全部做完"指令） | **D40 /trust 与 D41 /extensions 双落地（工单 16.4/16.5）**：16.4 收紧语义 = evaluateAll 后处理压 allow（未信任 cwd 下 bash/MCP 自动放行→逐次询问，deny/ask 不变）；16.5 声明式内容包（D18 不执行代码）+ symlink 逃逸拒载 + settings 名单启停。命令基线 20→22。同步：doc/02 v4.46、ARCHITECTURE v1.48、README v1.39、doc/08 v1.46。本批本机零验证 |
| v1.46 | 2026-09-12 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，"工单要全部做完"指令） | **§1 当前状态刷新：阶段十五全量收官（工单 15.1–15.4）**——15.1 spark mcp stdio server（ADR D39）、15.2 OpenAPI 导出（openapi.json + CI gen+diff；Python 生成物待外部需求）、15.3 @spark/skill-kit（skill 清单 schema 下沉 protocol 单一来源 + init/lint；typecheck 面 13→14）、15.4 Q-1 拍板维持纯声明 + MCP 兜工具面（doc/08 §15.4 收口块，受限可编程重开三触发条件登记）。本行只改当前状态句与版本表；§1.1 四端共享核与 §2 硬性约定不变。同步：doc/02 v4.47、doc/08 v1.47、README v1.40。本批本机零验证 |
| v1.47 | 2026-09-12 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起与决策：晚风（Wanfeng1028，"不允许在本机下载任何依赖包，写入规则"指令） | **§2 新增第 3a 条硬性约定：本机禁止下载依赖包**——不在本机执行 pnpm add/install/update 等任何会下载依赖的命令（本机零验证之下无此需要）；新增依赖流程 = AI 手改 package.json 声明 + 锁文件由人类执行或明示授权后生成（CI --frozen-lockfile 未同步前红在 install 步属预期中间态）。溯及注记：本会话 16.8 曾本机 pnpm add simple-git（规则立前），此后新依赖一律走新流程 |
| v1.48 | 2026-09-12 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（"工单要全部做完"指令） | **D42 /arena 多模型竞答落地（工单 16.8）**：InProcess 子会话 + git worktree 隔离 + 快照端点零新事件；胜者应用整体一次 fs.write 审批（删除类跳过——§2.10）；命令基线 22→23；同批实修 loadConfig 漏透传 agents/extensions 真 bug（D36/D41 勘误）。与 doc/02 v4.48、ARCHITECTURE v1.49、README v1.41、doc/08 v1.48 同批 |
...
**当前状态（编年史细节见 doc/02 §8 阶段表与版本行，本文件只留一句话）**：v1（阶段一~十）已全量合 main 并记入 CHANGELOG `1.0.0`；**阶段十一（可发布）~十五（生态面 15.1–15.4：spark mcp / OpenAPI 导出 / skill-kit / Q-1 收口）与阶段十七、十八已收官**；阶段十三（可证明）与阶段十四（SDK 化）全量落地；阶段十六 16.1–16.7 与 16.9 已落地（未完：16.8 /arena，并行在途）。**下一程顺序与依赖见 doc/08**，工单状态唯一来源是 doc/02 §8 阶段表（行首 `✅` = 已落地，无标记 = 未开工；每批完成在 doc/02 文末版本表追加 v3.x 行）。
...
3a. **本机禁止下载依赖包**（晚风 2026-09-12 拍板）：不在本机执行 `pnpm add` / `pnpm install` / `pnpm update` 等任何会下载依赖的命令——本机零验证（上条）之下没有需要依赖落地的场景，排查用命令全部依赖仓库既有 node_modules。**新增依赖的流程**：AI 手改 package.json 声明（版本写明确区间）并在提交说明注明"锁文件待 install 生成"，`pnpm-lock.yaml` 的重算由人类在本机执行或明示授权后进行；CI 的 `--frozen-lockfile` 在锁文件未同步前会红在 install 步——属预期中间态，不得为绿灯而在本机偷跑 install。
...
8. **测试**：`applyEvent` reducer 对全部事件类型逐一单测（27 种）；新增事件类型必须同步新增单测，否则 PR 不完整。
