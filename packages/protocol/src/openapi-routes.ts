...
export const OPENAPI_ROUTES: readonly OpenApiRouteMeta[] = [
  ...
  {
    method: 'get',
    path: '/api/storage/report',
    summary: '数据目录占用统计（阶段十九 19.37 第二批：~/.spark 按目录发现分桶；根不存在如实 exists:false）',
    tag: 'config',
    response: ref('StorageReportDto'),
  },
  {
    method: 'post',
    path: '/api/storage/cleanup',
    summary: '桶清理（阶段十九 19.37 第三批：白名单桶移入 trash，§2.10 只移不删；trash 桶为永久清空；白名单外 E_STORAGE_UNCLEANABLE）',
    tag: 'config',
    body: obj({ bucket: { type: 'string', minLength: 1 } }),
    response: ref('StorageCleanupDto'),
  },
  {
    method: 'get',
    path: '/api/storage/export',
    summary: '打包导出（阶段十九 19.37 第三批：全部会话 JSONL 逐字打包，marker 行 + 原始行）',
    tag: 'config',
    response: ref('StorageExportDto'),
  },
  {
    method: 'post',
    path: '/api/storage/import',
    summary: '回导（阶段十九 19.37 第三批：未知会话按原文件落盘、同名跳过、坏段计数；落盘后重建会话索引）',
    tag: 'config',
    body: obj({ bundle: { type: 'string', minLength: 1 } }),
    response: ref('StorageImportDto'),
  },
  {
    method: 'post',
