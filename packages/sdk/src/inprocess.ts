...
export class InProcessTransport implements Transport {
  ...
  /** 桶清理（19.37 第三批）：引擎公共面直透（InProcess 对等） */
  storageCleanup(bucket: string): Promise<StorageCleanupDto> {
    return this.engine.storageCleanup(bucket)
  }

  /** 打包导出（19.37 第三批）：引擎公共面直透（InProcess 对等） */
  storageExport(): Promise<StorageExportDto> {
    return this.engine.storageExport()
  }

  /** 回导（19.37 第三批）：引擎公共面直透（InProcess 对等） */
  storageImport(bundle: string): Promise<StorageImportDto> {
    return this.engine.storageImport(bundle)
  }
  ...
  promptsInfo(): Promise<PromptsDto> {
    return this.sync(() => this.engine.promptsInfo())
  }

  /** 写槽位模板（阶段十九 19.18）：引擎同步方法（原子写盘）——走 sync 门 */
  updatePrompt(update: PromptsUpdate): Promise<PromptsDto> {
    return this.sync(() => this.engine.updatePrompt(update))
  }
