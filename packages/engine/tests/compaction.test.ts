 ...
 * generateOnce 调用形状（压缩提示词+转录 / maxTokens 2000）、keptFromSeq 尾部
 * 预算反推（最新一条必保/不越旧锚点）、二次压缩锚点级联、LLM 失败不杀 turn、
 * run-loop 集成（触发→压缩→重投影→采样看到摘要上下文）。
 */
...
describe('keptFromSeq 尾部预算反推（§5.8.5 "N 由 token 预算反推"）', () => {
  /** 5 条 user 消息（seq 1..5），每条 data 序列化长度相同 */
  ...
  test('预算充足：全部保留（keptFromSeq=首条 seq）', async () => {
    const f = makeFixture({ keepTokens: 100_000 })
    await fiveMessages(f)
    f.gateway.scriptOnce('S')
    await f.compactor.compact()
    expect(lastEvent(f, 'compaction.completed')?.data).toMatchObject({ keptFromSeq: 1 })
  })

  test('预算只够尾部：keptFromSeq=尾部首条 seq（投影只剩尾部+摘要）', async () => {
    // 每条消息 data JSON ≈ {"text":"消息零一二三四五六七八九N"} ≈ 22 字节 → ~6 token
    const f = makeFixture({ keepTokens: 12 }) // ≈ 最近 2 条
    await fiveMessages(f)
    f.gateway.scriptOnce('S')
    await f.compactor.compact()
    expect(lastEvent(f, 'compaction.completed')?.data).toMatchObject({ keptFromSeq: 4 })
  ...
  test('预算极小：最新一条无条件保留（不把当前上下文全部摘要掉）', async () => {
    ...
    f.gateway.scriptOnce('S')
    await f.compactor.compact()
    expect(lastEvent(f, 'compaction.completed')?.data).toMatchObject({ keptFromSeq: 5 })
  })

  test('二次压缩：预算再大也不越过旧锚点（已摘要事件不复活）；旧摘要不重复出现', async () => {
    // 第一次压缩：预算 12 → 只保留尾部（keptFromSeq=4）
    ...
    // 第二次压缩：预算放大到全量——边界仍被旧锚点 4 卡住（事件 1-3 已被摘要）
    ...
    // 投影：新摘要 S2 + 锚点 4 之后的事件（S1 不是 surface 事件，不重复出现）
    const ctx = f.projector.modelContext()
    const texts = ctx.messages.flatMap((m) =>
      m.content.filter((c) => c.type === 'text').map((c) => (c.type === 'text' ? c.text : '')),
    )
