...
describe('isBlockedAddress（SSRF 黑名单矩阵）', () => {
  const blocked = [
    ...
    '::ffff:127.0.0.1',
    // 撤 ::ffff:0:0/96 后（整段 = 全部 v4 会误杀公网，见 ssrf.ts），mapped 私网靠 Node BlockList
    // 的 v4-mapped 归一化命中既有 v4 私网段——点分与十六进制两种写法都须仍被拦（固化该不变量）
    '::ffff:192.168.1.1', // 点分 mapped → 192.168.0.0/16
    '::ffff:a01:101', // 十六进制 mapped（= 10.1.1.1，ssrf.ts 注释声称的变体）→ 10.0.0.0/8
    '::ffff:c0a8:101', // 十六进制 mapped（= 192.168.1.1）→ 192.168.0.0/16
