...
| v1.1 | 2026-09-21 | AI 编写：Qoder；发起：晚风（Wanfeng1028，"不只是工单19，之前的也要审查……指出是谁写的不好"指令） | 补 **§4.10 全阶段"声称-vs-落地"对账（38 单抽样：证真 23／打折 12／证伪 3）** + 失败模式 A/B/C + **§5 归属分布表**（含本会话同标识 Qoder 的 4 条证伪，采"列出证据、不自行定责"处理）；新增 **LA-63~LA-68**（未执行 grep 验收加闸、✅ 回收纪律、批量 lift 禁照抄目标文本、逐项修文本、两处 `as never`、WO-097/098 重号）。§7 改为回收说明 |
| v1.2 | 2026-09-25 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，"已经完成的工单有问题的要修复……把所有的工单都做完"指令） | **LA-01/02/03 收口（§6.1 安全五张的前三张；doc/02 v4.122、ARCHITECTURE v1.66 同批）**：① evaluate 层间 deny 优先（§4.1 P0 的根：项目 allow 覆盖用户 deny 不再可能）；② trust 收紧面 +fs.write/agent.task/computer.use；③ 项目层按会话 cwd 惰性建层 + 未信任信任门（整层不进评估 + warn/审计）+ 家目录撞路径不设层 + 固化/级联按会话层走；④ E_PERMISSION_SCOPE 进 errors.ts 与 err.* 文案表。验收对账：LA-01 三条（不可信 cwd 项目 allow→ask、trusted 后生效、装载条数进审计流）与 LA-02 两条（项目 allow 压用户 deny 必判 deny、规格行与代码一致）由 permission-la.test 13 例承载；LA-03 三条（跨会话固化不串、$HOME 场景拒固化、HTTP 形状 409 非 500）同批承载；LA-04（规则可查看/可撤销）与 LA-05（装/换可执行审批链）未做，继续在册 |
| v1.3 | 2026-09-25 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，同上指令全程） | **LA-04 收口（doc/02 v4.124 同批）**：`PermissionRuleDto.source` 可选字段（引擎合成）+ `listPermissionRules` 双层合成（defaultCwd 项目层；多 cwd 逐项管理登记为限制）+ `removePermissionRule` 增 scope（项目规则内存与文件同步删、审计 source settings-ui:project）+ web 规则页来源徽标与按源删除 + mock/transport 三通道对等。验收对账：DTO 契约用例重跑（gen:contract 133 describe/1295 断言）；四端可见可删——web 已接，cli/mobile/miniapp 无规则管理页（该面只有 web 设置中心，与用户级现状同形，不另扩）。LA-05 仍待做，继续在册 |
| v1.4 | 2026-09-25 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，同上指令全程） | **LA-05 收口（doc/02 v4.125 同批）——§6.1 安全五张（LA-01~05）全部清账**：非环回确认门（403 E_CONFIRM_REQUIRED + 完整命令回显，lsp 走 protocol 清单单源、mcp 逐条回显 stdio 命令），环回缺省行为不变红线保持；computer.use 收紧随 LA-02 覆盖。验收对账：非环回无确认 403、环回不变两验收点由 confirm-gate-routes.test 6 例承载；"引擎内审批"改为"confirm 门 + 命令回显"（server 层无会话语境，挂起卡模型对不上——同等安全目标、更小形状，判词偏离已在此登记） |
| v1.5 | 2026-09-25 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，同上指令全程） | **LA-06/07/67/68 收口（doc/02 v4.127 同批）**：LA-06 拼错命令修正 + ENOENT 归类 + 可执行名封闭集断言测试（验收原话"常量断言测试"落地为源码扫描式封闭集）；LA-07 幽灵通道删除并改真实 skipIf（验收"用例存在且 CI 真跑"达成）；LA-67 两处 `as never` 清零（第三处 mock-parity 的同型强转顺带清）；LA-68 重号照两张 D28 判例主题消歧。**LA 在册余量**：LA-08~21（computer/bash 测试与功能面）、LA-22~26（契约/arena）、LA-28~41（引擎核心残留）、LA-43~51（四端外壳）、LA-52~62（协议共享核与 Web）、LA-63~65（登记纪律闸）——LA-01~05/06/07/27/42/56 已清 |
| v1.6 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，"继续"指令） | **LA-08/11 收口（doc/02 v4.133 同批）**：LA-08——computer 三平台纯函数导出 + 零 GUI 补测 13 例（含工厂路由 mock platform 与 Unsupported 降级），**括号污染实证并根治**：macOS window/app 清单从 AppleScript 列表 ", " 分列改行式 0x1F 分列（标题含逗号不再破坏解析）+ `name of front window` 逐进程 try 容错；LA-11——AGENTS.md §2 第 13 条硬性约定（批量编辑逐处断言 + 多转义内容走临时脚本文件）。**LA 在册余量**：LA-09/10/12~21（bash 池与 MCP 测试及功能面）、LA-22~26、LA-28~41、LA-43~51、LA-52~62、LA-63~65——LA-01~08/11/27/42/56/67/68 已清 |
| v1.7 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，"继续"指令） | **LA-09/12~16 收口（doc/02 v4.134 同批）**：LA-09 补测 9 例（含超时重建与迟到定时器不误杀回归）；LA-12 spawnCwd 显式落会话根 + homeCwd 诊断；LA-13 哨兵 nonce + 前导换行（无尾换行命令不再卡死）；LA-14 池内执行期收集上限 + StringDecoder + 残行缓冲上限；LA-15 身份比对出池 + busy 豁免 + 构造期 error 注册 + stdin error 吞并 + 出池不挂定时器；LA-16 引擎 shutdown drain + 主开关关闭排水 + ADR D45 语义修正（"零注入面"→"命令体裸写、哨兵可被 heredoc 吞"如实登记）。**LA 在册余量**：LA-10（MCP 环回正例）、LA-17~21（computer-use 面）、LA-22~26、LA-28~41、LA-43~51、LA-52~62、LA-63~65——LA-01~09/11/12~16/27/42/56/67/68 已清 |
| v1.8 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`） | **LA-17/21 收口（doc/02 v4.135 同批）**：LA-17 三执行体入口 signal.aborted 预检（screenshot/click/type/key 四操作覆盖）；LA-21 Linux app launch detached+unref 立即返回。**LA 在册余量**：LA-10（MCP 环回正例）、LA-18/19/20/21 尾（剩余操作预检）、LA-22~26、LA-28~41、LA-43~51、LA-52~62、LA-63~65——LA-01~09/11/12~17/21部分/27/42/56/67/68 已清 |
| v1.9 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`） | **LA-10 收口（doc/02 v4.138 同批）**：MCP streamable-http 分支可达性测试——`defaultTransport` 导出 + 测试验证 streamable-http 配置产出 StreamableHTTPClientTransport 实例且 stdio 配置不产生该类型。**限制如实登记**：正向连接测试（本地 MCP server + 完整 JSON-RPC 握手 + 自定义 header 注入验证）需真实网络 + 完整 MCP 协议实现，超出静态测试能力——真网关联调留用户。**LA 在册余量**：LA-18/19/20 尾、LA-22~26、LA-28~41、LA-43~51、LA-52~62、LA-63~65——LA-01~17/21部分/27/42/56/67/68 已清 |
| v1.10 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，转 gpt-6-astra 外部评审） | **外部评审（gpt-6-astra）交叉验证**：对 00f074b（2026-09-24）做独立静态审计，发现 7 项问题。与本报告对照：4 项已被 LA-06/07/12/13 覆盖（评审者审计的是修复前的提交）；**3 项新发现**注册为 GT-1（resourceOf 80 字符截断授权标识）/GT-2（reply 级联放行未过 trust 收紧）/GT-3（Arena applyWinner 只删行误判删文件）；另 GT-4（npm.cmd spawn）/GT-5（HTTP_PROXY 类型不匹配）。评审者还确认了本报告的核心结论——"完成标准混乱""测试绕开最普通边界""注释比代码更自信"——独立来源的交叉验证是有价值的 |
| v1.11 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`） | **LA-17/21 收口 + 19.45 沙箱四级第一批落地（doc/02 v4.140 同批）**：LA-17 三执行体 signal.aborted 预检（screenshot/click/type/key）；LA-21 Linux app launch detached+unref；19.45 沙箱从 'off'|'on' 扩为四级（off/approval/light/heavy）+ heavy 模式（bwrap --unshare-net / Seatbelt strict profile）+ 全仓 'on' 引用清理 8 文件。**LA 在册余量**：LA-18/19/20 尾、LA-22~26、LA-28~41、LA-43~51、LA-52~62、LA-63~65——LA-01~17/21部分/27/42/56/67/68 已清 |
| v1.12 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`） | **LA-25/33/41 收口 + LA-28/22 收口（doc/02 v4.139~140 同批）**：LA-25 Arena isRecord 补 prompt/contenders 校验（TS2339 红修）；LA-33 settle error void→.catch()（abort/timeout 路径不再吞异常）；LA-41 AGENTS.md engine.ts 行数口径更新（LA-41 实测）；LA-28 edit $ 展开修复（`replace(a,b)` → `replace(a,()=>b)` 防 `$&/$1` 展开）；LA-22 openapi /api/mcp/config 路径补登。**LA 在册余量**：LA-10尾/18/19/20尾/23/24/26/29~37/39/40/43~51/52~62/63~65——LA-01~09/11/12~17/21部分/22/25/27/28/33/41/42/56/67/68 已清 |
| v1.13 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`） | **LA-26 收口（doc/02 v4.141 同批）**：Arena boot running 改判 interrupted——`ArenaHistoryEntryDtoSchema.status` 枚举加 `'interrupted'` + history() 投影 `running→interrupted`（boot 后进程重启，running 状态已失真）。openapi.json 重跑同步。**LA 在册余量**：LA-10尾/18/19/20尾/23/24/29~37/39/40/43~51/52~62/63~65——LA-01~09/11/12~17/21部分/22/25/26/27/28/33/41/42/56/67/68 已清 |
| v1.14 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`） | **LA-29/30/31/32/35 五张收口（doc/02 v4.142 同批）**：LA-29 成本熔断接活——models.json 模型条目可选计价四率（USD/Mtok，pi-ai 口径）/maxTokens/imageInput 经 ModelRef→ResolvedModel→toPiModel 进真网关装配（原硬编码 0/8192/['text']，美元维度永不触发），CostTracker.exceeded 加 token 维度兜底（costLimitTokens，全 token 含 cache 两分量 ≥ 阈值即熔断），RoutingDto/RoutingUpdate/UsageSummaryDto 三 DTO 扩字段 + web 设置页/CLI /usage 面板/mock 三端对等，persistRouting toRef 保留计价字段（防 PUT 写回剥掉已声明定价）；LA-30 悬空 toolCall 闭合——run-loop 抽 closeDanglingCalls，maxSteps/aborted/budget 三收尾路径补 E_ABORTED started/completed 对 + toolResult 回喂（防 UI 永转圈 + 下轮 API 无配对 tool_use 被拒）；LA-31 forkSession 边界判空重序（边界自身 type 先计入再判空——turn.completed 处由误拒改合法、turn.started 处由漏判改拒绝）；LA-32 硬边界统一——writeFileInCwd 与附件读盘都改 resolveInRoot（realpath+relative；原 startsWith 词法检查被同级前缀目录 `../proj-evil` 绕过）；LA-35 缺尾换行修复档——read 末行完整 JSON 无尾换行标 tailTorn fix='appendNewline'，resume 补换行续写（原实现续写粘连成残缺 JSON，崩溃残留升级为持续损坏）。验收对账：pi-gateway 计价装配两例 + cost-tracker token 兜底两例 + run-loop 悬空闭合 + store 缺尾换行 + engine fork 两形态；生成物重跑 openapi（74 路径/61 组件）+ 契约（140 describe/1375 断言）。**LA 在册余量**：LA-10尾/18/19/20尾/23/24/34/36/37/39/40/43~51/52~62/63~65——LA-01~09/11~17/21部分/22~33/35/41/42/56/67/68 已清 |
...
| 全阶段对账（38 单抽样） | **代码可信 > 文档声称可信** | 证真 23／打折 12／证伪 3；"AI 实现造假"0 例，但约 1/3 工单的登记声称与代码有可核对偏差；mock 对等与测试真实性两项守得最好 | 见 §4.10 归属分布表 |
...
### 4.10 全阶段"声称-vs-落地"对账（38 单抽样）

**分布**：证真 23（60.5%）／部分落实·打折 12（31.6%）／证伪 3（7.9%）。

**核心结论**：**代码的可信度显著高于文档声称的可信度。** 三条证伪里只有 1 条是"代码没做到却勾了"，另两条属**文档搬运过程失真**。按"AI 实现本身造假"算本轮 **0 例**；按"回填登记的声称与代码不符"算约 **1/3 工单有可核对的偏差**。

三个正面结论同样要记：**① mock 当真落地这一类基本干净**——19.10 自标"静态两场演示"、19.11 自标"静态一笔＋延迟模拟"、mock 缺口被单独立成 19.22（⬜），三通道 75 方法对等零缺口，是守得最好的一项纪律；**② 没有空心测试**——0 个 `expect(true)`/`todo`，3 处 skip 全是 `skipIf`（CI 真跑本机 skip），低频事件（`session.resumed`/`session.title`/`memory.injected`/`goal.updated`/`checkpoint.created`）逐条是真 `applyEvent + expect`，测试用例数声称与文件实测**逐条吻合**；**③ AUD-01~14 十三个哈希全存在且抽核为真实现**（ZCode · Union Alpha 那一批全证真，0 打折 0 证伪）。

#### 证伪与打折详解（本会话已复核实据）

| # | 发现 | 证据 | 归属（自报） |
| --- | --- | --- | --- |
| D1 | **19.12「设置中心占位页余 0」＝回填虚高**（最严重） | `[读码]` 三处声称 `doc/02:2831`(v4.76)、`doc/08:31`(v1.62)、`doc/08:2748`；实际 `settings-pages.ts` 的 `onboarding` 仍 `placeholder`，消解它的 19.15 至今 ⬜。加重情节：同一批自己的 doc/08 §5D 头注记就把引导页列为待消解占位之一，`doc/02:95` 也逐项列过"六占位页"——**五消一余却写余 0**；登记提交 `dc223a3` 是**纯文档 5 行零代码**，虚报完全发生在回填环节 | **ZCode CLI · GLM-5.3-Flash** |
| D2 | **16.2「四端面板」＝lift 阶段凭空加高措辞** | `[读码]` 本会话复核：`doc/02:2921` 确有"四端面板"，而 `grep -l listAgentPresets\|AgentPresetDto apps/mobile/src apps/miniapp/src` = **0 个文件**；原 doc/08 卡片与落地登记只写"web 页面 / `/agents`"。"四端"三字**只存在于 doc/02**，产生于 `70bc068`（一次性把阶段十二~十八 48 张工单从 doc/08 批量 lift 建表） | **Qoder**（lift 步） |
| D3 | **R-D「2028 → 约 600 行门面」＝把立项目标当成果照抄** | `[读码]` 本会话复核：`doc/02:2941` 确有"约 600 行门面"；R-D 收口时实测 **1499** 行（doc/08 v1.7 诚实记了这个数），HEAD 实测 **2257** 行——比拆前起点还大 229 行。权威表抄的是卡片目标文本 | **Qoder**（v4.54 表行）；真数记录者 GLM-5.3-Flash |
| D4 | **阶段十八的 grep 类验收＝未执行即勾选（两处）** | `[读码]` 本会话复核：`components/ui/dialog.tsx:44` 确含 `shadow-lg`，而 `DESIGN.md:271` 把 `shadow-lg`/`shadow-2xl` 明列为 §12.8 零命中检查项 → 18.2"grep 零未登记档位"与 18.5"§12 六类黑名单全量自查零命中"均不成立；且 doc/08 §18.5 进度条目逐条列的是 AA/e2e/文档/ADR，**没有 §12 grep 的执行痕迹**。另 `rounded-md`/`rounded`/`rounded-sm` 三处在 18.4 宣布清零后被后续提交重新引入，**那三个提交无版本行署名 → 归属不明** | **Qoder**（18.2 `f6543545`、18.5 `f612a6f`/`70bc068`） |
| D5 | **13.1「`examples/evals/src/tasks/` 17 场景」＝计数归属层级错位** | `[读码]` 该目录实为 **13** 条（自建立全史未变），17 只有并上 `scenarios/` 的 4 个 core 才成立——数没错，**挂错目录**（`nightly.yml:24` 的混合口径正是此源） | **Qoder** |
| D6 | **19.2 两处"如实"其实不如实** | 见 §4.2：`screapture` 拼错被一并抄进文档故"看起来自洽"；Wayland 无判定代码，实际靠"缺工具"路径偶然覆盖，不是宣称的显式拒绝 | GLM-5.3-Flash（19.2 三手经手） |
| D7 | **14.1「16 值导出，与不变量网白名单逐字对应」已不成立** | `[读码]` 实测公共值导出 **18**、`public-surface.test.ts` 白名单亦 18；第 17/18 项由 `d909635`(RT3-07) 加入，**改了测试没回改 doc/02 清单标题数** | 计数 **Qoder**；漂移 GLM-5.3-Flash |
| D8 | 19.12 的收窄未按自家规矩登记 | 卡片写"预置**编辑面**"、实现是只读视图；同批 19.11 老实写了"工单内收窄（偏离登记）"，19.12 无 | GLM-5.3-Flash |
| D9 | 12.5 端点路径与实现不符 | 工单写 `GET /api/fs/tree`，实为 `GET /api/sessions/:id/fs/tree`；`check_doc_links.py` 只校仓库路径不校 API 路径故抓不到 | Qoder |
| D10 | 6.6「用量条」勾选已失效 | `UsageBar.tsx` 已停用为死文件（10.5⑦ 判决有登记、进了 `knip.jsonc` ignore），但 ✅ 行文本从未更新。**形态是"勾选不回收"，不是虚报** | 原勾 Trae · GLM-5.3；改判 Qoder |
| D11 | `doc/10` WO-097/098 **重号** | §13 round5 收编与 §14 DSH 两套表各占一次，两提交自报同一署名，无法分辨谁该改名（与 AGENTS v1.29 登记的"两张 D28"同类卫生缺陷） | 归属不明 |

#### 失败模式（比个人归因更有解释力）

- **模式 A：一次写对、此后无人再校。** `shadow-lg`、`rounded-*`、`engine.ts` 行数、"16 值导出"、6.6 用量条——勾选当时成立，之后被后续工单打破而 ✅ 不回收。**这些指标没有一条在 CI 关卡里**（`check_doc_links.py` 只锚事件数与速查条数两个数），全靠自觉重跑，而自觉从不发生。
- **模式 B：把目标文本当成果文本搬运。** 批量 lift 的病。四条最重证伪里**三条（R-D、16.2、13.1）产生在 `70bc068` 那一步**，与"某个模型写代码偷懒"关系不大。
- **模式 C：文档比代码更乐观。** 值得注意的现象是**同一会话常常一边诚实一边夸大**：19.11 认真写了偏离登记、同批 19.12 就没写；v4.70 认真注记"19.4 子代理起草、主会话补完"、v4.65 却把 `screapture` 一并抄进声称。不是知识缺失，是**回填时对"声称句"这一格没有和"证据句"同等对待**。

#### 归属分布（自报元数据，非取证）

| 归属 | ✔ | ◐ | ✘ | 主要证伪项 |
| --- | --- | --- | --- | --- |
| ZCode CLI · GLM-5.3-Flash | 22 | 5 | 3 | 19.12"余 0"、19.2 两处"如实"、14.1 清单数漂移 |
| **Qoder**（=本会话所用标识） | 24 | 6 | 4 | 18.5 grep 未执行、18.2 `shadow-lg`、16.2"四端面板"、R-D 照抄、13.1 计数 |
| ZCode · Union Alpha | 14 | 0 | 0 | — |
| Trae · GLM-5.3 | 1 | 1 | 0 | 6.6 陈旧勾选 |
| 归属不明/待核 | — | 3 | 2 | 三处 `rounded-*` 回归提交、两处 `as never`、WO-097/098 重号 |

**必须一起读的四条局限**：① 按"风险优先"选题，偏向阶段十九/十八/十七与可 grep 硬指标，**非随机抽样，不能据此推全项目虚报率**；② 阶段一~五只零星触到（那几段已被 doc/05 审过一轮）；③ 自报署名可照抄可伪造（AGENTS v1.15 就发生过一次），"谁"只到"哪个工具·模型的会话自述"；④ 并行同日交叉时同批多署名并存仍可能错配。

**本报告自身的立场**：执行本审计的会话自报标识同为 **Qoder**，上表最重的一栏里就包含自己的标识。这不构成回避理由，也不构成定责依据——它是自报元数据，且 D2/D3 两处失真发生在**搬运他人文本**的 lift 步骤而非本会话的写码步骤。处理方式与 §1.4 一致：**列出来，让证据说话，不由本会话裁定归属结论**。

### 6.10 登记卫生与回填纪律（对账组）

| # | 内容 | 落点 | 验收 |
| --- | --- | --- | --- |
| LA-63 | **给"未执行的 grep 验收"加闸**：§12.8 检查项清单、圆角封闭集、`public-surface` 值导出条数、`engine.ts` 行数、端点路径字符串——做成第二关（扩 `scripts/check_doc_links.py` 或新增 checker），使"声称零命中"必须有当次执行痕迹 | `scripts/check_doc_links.py` 或 `scripts/check_invariants.*` | 上述五项任一偏离即 CI 红；`dialog.tsx` 的 `shadow-lg` 当场被抓 |
| LA-64 | **✅ 回收纪律**：凡推翻既有交付物的工单（10.5⑦ 之于 6.6、RT3-07 之于 14.1），必须在**同一提交**里回改被推翻那行的文本；写进 `docs-update` SKILL 第 3 步交叉引用同步 | `.agents/skills/docs-update/SKILL.md` | SKILL 有该条；6.6 与 14.1 两行按实况改写 |
| LA-65 | **批量 lift 禁止照抄卡片文本**：doc/08 → doc/02 的工单表迁移必须分"立项目标"与"验收实测"两栏，禁止把目标数字（"约 600 行"）与宽口径（"四端面板"）搬成成果声称 | `doc/02` §8 阶段十二~十八表、`AGENTS.md` §2 文档纪律 | R-D / 16.2 / 13.1 三行按实测改写；lift 流程有文字规矩 |
| LA-66 | 逐项修文本：R-D 行改"1499 行（现 2257，见 LA-41）"、16.2 改"web/cli 两端"、13.1 改"13 场景（tasks/）+4 core（scenarios/）"、14.1 改"18 值导出"、12.5 改真实端点路径、19.12 补"工单内收窄（偏离登记）：`url:` 视图为只读" | `doc/02:2941,2921,2876,728` 与 §4.5 表、`doc/08` 19.12 卡 | 每行可与源码复现 |
| LA-67 | 消两处 `as never`（`apps/cli/src/hooks/use-session-stream.ts:28`、`packages/engine/src/automation/manager.ts:96`）——10.27 已为这类强转立过规矩"绕过类型即绕过校验语义" | 两文件 | 改窄类型或 `unknown` 收窄；`as never` 全仓归零 |
| LA-68 | 修 `doc/10` WO-097/098 重号（§13 round5 与 §14 DSH 两套表各占一次），照 AGENTS v1.29"两张 D28"的消歧注记格式处理 | `doc/10` §13/§14 | 编号唯一或有显式消歧注记 |
...
九个分组已全部回收：阶段十九 9 单、引擎核心、服务端与四端外壳、协议+Web、官网、全阶段对账（§4.1~§4.10）。§1.1 的范围声明已覆盖实际审查面。

后续若对本报告作增补，在版本表追加行、LA 编号继续往后排，不复用已发号。
