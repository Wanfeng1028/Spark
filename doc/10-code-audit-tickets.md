...
| v1.19 | 2026-09-24 | AI 编写：Qoder · Qwen3.8-Max（Qwen3.8-Max-0902）；发起：晚风（Wanfeng1028，"把官网部署在 github pages"指令） | **§15 official 四单收口（WO-111/112/113 全修、WO-114 部分修）+ 官网发 GitHub Pages**：① WO-111 取"递归提取"分支——`ProtocolSection.tsx` 新增 `textOf()`，不再按 JSX 固定深度取值；② WO-112 示例改 `events.subscribe`（源码核：`packages/sdk/src/client.ts:64` 声明、`:107` 装配到 `transport.onEvent`）；③ WO-113 四处按源码实数改：内置命令 **23→28**（权威断言 `packages/protocol/tests/commands.test.ts:49`）、Transport **67→84**（`interface Transport` 成员去重实数，**非本单记的 69**）、审批键位两处（`SessionDemo`/`SessionDemoZone` 的 `[a]/[A]/[r]` → `[y]/[a]/[4]/[n]` + quickstart 四键映射）、`npm i -g @spark/cli` 三处 CTA 改源码话术（`gh api releases` 空数组 + release.yml 唯一 run 红于 ENEEDAUTH，包确未发布）。**本单自判过时一处**：②③称"产品审批实为三值故 1/2/3/4 四键是错的"——19.9/ADR D48 之后 web ApprovalCard 确有第四按钮「本项目总是允许」、CLI 有 4 键，**四键是对的，错的是内容映射**（事实源 `apps/cli/src/components/ApprovalPrompt.tsx:22`）。另记两处新发现：该错文案源头在 **README.md:36**（官网由此抄去，同批改并挂 README v1.45）；`official/src/lib/constants.ts` 头注原写"CI 校同步"是假声明（check_doc_links 不扫 official/），已改为"手工抄自源码实数"。④ WO-114 部署前置已做：`next.config.ts` 补 `basePath`（env 驱动缺省 `/Spark`）+ `trailingSlash: true`、`asset()` 前缀 10 处裸 `<img src>`/metadata 路径、新增 `official/public/.nojekyll`（防 Jekyll 丢 `_next/`）、`official.yml` 增 deploy job；**同批停用 `release.yml` 的 docs job**（一仓一 Pages 站且 deploy-pages 整站覆盖，晚风拍板官网先占 `/Spark/`）；剩 `ThemeProvider` 的 `enableColorScheme` 一枚未修。本批本机零验证，CI 裁决 |
...
| WO-111 | official | **P0** ✅已修 2026-09-24 | 开发者区代码窗 Copy 按钮复制出的文本缺全部代码行——只剩注释与 `});` | plainText 提取只下钻一层：TS_LINES 的 l0/l2/l4 是 Fragment 包 span（span 文本在第二层），`typeof c === "string"` 对 span 全 false → 整行提取为空串（official/src/components/sections/ProtocolSection.tsx:45-47,77-92；已抽核坐实） | 递归提取文本，或 TS_LINES 每行同时保存 `plain` 字符串字段 |
| WO-112 | official | **P0** ✅已修 2026-09-24 | 首页示例代码调用不存在的方法——`client.events.onEvent(...)` 复制执行即 TypeError | SDK 便利分组实际面是 `events.subscribe`（packages/sdk/src/client.ts:61-72,104-106；已抽核坐实）——违反官网"代码为真实 API 面"自律 | 示例改 `client.events.subscribe((envelope) => {…})` |
| WO-113 | official | P1 ✅已修 2026-09-24（本单自判有一处过时，见 v1.19 版本行） | 事实漂移四处（对着源码同步一批）：① 统计行「内置命令 23」实为 24（19.2 新增 computer）；② 快速上手审批键位「1/2/3/4 四键」产品实为三值（once/always/reject，CLI y/a/n）；③ 首屏演示终端键位 `[a]批准一次 [A]总是 [r]拒绝` 同错；④ features 页「67 个方法」实为 69 | official/src/lib/constants.ts:11；quickstart/page.tsx:188-189；SessionDemoZone.tsx:117-122；features/page.tsx:99（对照 packages/protocol/src/commands.ts 24 条、keymap.ts、transport.ts 69 方法——①已抽核坐实） | 一次批对照源码改常量与文案；FACTS 计数纳入 check_doc_links 同步面（或注释改"以源码为准"去掉假"CI 校同步"声明） |
| WO-114 | official | P1 部分已修 2026-09-24（basePath/trailingSlash 与三处 CTA 已改；**ThemeProvider 的 `enableColorScheme` 一枚未修**，不在本轮授权范围内） | 主 CTA `npm i -g @spark/cli` 现状 404（包未发布）；metadataBase/canonical 指向 GitHub Pages 项目页但未设 basePath，真部署时 `/_next/*` 资源与站内链接全 404；另 P3 一枚：单主题亮色站挂 `enableSystem` 的 next-themes Provider，系统深色用户被注入 `color-scheme: dark`（深色滚动条/原生控件出现在纯亮色页面） | registry 实测 404（tag v1.0.0 已打、发布未落地——发布窗口期问题）；official/next.config.ts:3-8 与 layout.tsx:9 两处口径未联动；components/layout/ThemeProvider.tsx:14-23（tokens.css 无 dark 变量、class 无消费方，仅 color-scheme 副作用可见） | 发布落地前：CTA 改源码安装话术或挂「即将发布」态；部署到项目页时同步设 basePath/assetPrefix，或 SITE_URL 改根域名；ThemeProvider 去掉或 `enableColorScheme={false}`。均发布/部署前置条件，随发布批处理 |
