...
| ---- | ---- | ---- |
| push main | `check_doc_links.py` → `pnpm typecheck` → `pnpm lint` → `pnpm knip` → `pnpm test` → `pnpm eval` | eval 冒烟自工单 13.1 第三批进主 CI（doc/08 §13.1 验收第 1 条与提示词第 4 条："CI（非 nightly）用 ScriptedLlm 冒烟"）：core 四场景 + 任务级判分双向自检 13 场景，无 key 无网络约 10s。**knip 自工单 14.1 第二批进主 CI**（ARCHITECTURE §9.6 "未引用导出/依赖"硬检查项）：lint 后的**独立一步**而不并进 `pnpm lint`——两个工具报告形状与失败语义不同，混在一起会让 eslint 红灯被 knip 噪声掩盖；配置在根 `knip.jsonc`（每处 entry/ignore 原地写理由），首份报告 86 项裁决表见 doc/02 §4.6.3 |
