...
| L3 E2E | 用户旅程（mock 四场景 + 真实 server 冒烟 + 断线） | Playwright（chromium 单浏览器，禁多浏览器矩阵；**locale 钉 `zh-CN`**） | apps/web/e2e/ | 阶段六工单 6.8 首批。locale 为 19.17 后补：全新配置无 `ui.language`（engine/config.ts 该键 optional 且无缺省），I18nProvider 落到 `detectLanguage(navigator.languages)`，而 chromium 缺省 locale 是 en-US ⇒ 走字典的串（连接态/错误人话）渲染成英文，用例断言中文即红 |
...
**Transport 接口契约套件（工单 14.2 第二批已落地）**：`apps/server/tests/transport-contract.ts` 导出 `transportContractSuite(name, makeChannel)`——**参数化**的传输层语义套件，任何 Transport 实现都跑同一组断言（会话生命周期与归档过滤 / 未知会话 E_NOT_FOUND 错误码同形 / 回合直播与回放一致且 seq 升序 / 退订后不再投递）；HTTP 通道已接（`transport-http-contract.test.ts`：真实 listen + ScriptedLlm + HttpTransport），**InProcess 通道随 14.4 接同一份**——双通道 parity 由此成立，而不是各写各的测试。套件刻意**不管**三件事：SSE 时序/心跳/背压（`sse.test.ts` 手写领域）、DTO 形状（protocol 生成物）、审批与工具语义（evals core 套，通道无关）；`replyPermission` 的路由 parity 待 14.4 两个实现同时在场时补进套件（那时断言才有对照意义）。

**路由级 payload 生成（①合法 payload 打路由断言 2xx / ②非法 payload 断言 400）归 15.2 OpenAPI**，理由（第二批核对后的裁决）：它需要一张**路由 × body schema 的映射表**，而那张表是 server 的知识、protocol 里没有（硬要求就是把它造出来）；而 400/404/409 的映射语义已被 `apps/server/tests/routes.test.ts` 手写覆盖（它断言的是 statusCode + `code` 字段，属路由映射层而非 DTO 形状层）。15.2 落 OpenAPI 时这张映射表本来就是必需品，届时生成路由级契约用例零额外成本。据此 **§4 的"server 重复部分由契约生成物替代"收敛为：不替代**（层次不同：生成物管 schema 形状，手写件管路由映射与时序）。
