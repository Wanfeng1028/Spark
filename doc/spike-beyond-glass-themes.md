...
1. **视频素材策略**（第二批阻塞项）：① 运行时热链 Coverr（联网依赖 + 离线降级）② 打包进仓（体积 + 需 §2.3a 下载授权）③ 用户自备本地文件。**附带判据**：上游热链失败时是 `play().catch(() => {})` 静默回落到静帧（`index.html:1312` 注释"Offline: the wash still carries the wallpaper"）——**Spark 不得照抄这个静默**，必须如实报错（失败闭合）。
2. **主题是否可影响 accent**：建议**不允许**（§5 第 2 行）。若允许，等于让壁纸改写产品身份色，需晚风单独拍板并修 §12.9 边界①。
3. **亮色默认下的 scrim 定标**：§6.6 的 `rgba(4,7,13,…)` 是暗色页数值，Spark 亮色为默认，需重新实测对比度后定值。
4. **静帧兜底图的来源**：上游 `.wash` 用的是 `thumbs/`（9–37KB 小图） stretched 到 116% 视口 + blur(34px)——**低分辨率能过关正是因为它被糊掉了**。Spark 不模糊，所以 WebGL 不可用时的兜底需要另一套方案（纯色 / 渐变 / 或不提供兜底而如实提示），不能直接用小图。
