...
* **第二批进度（2026-09-10：`exit_plan_mode` 工具 + `/plan` 命令，即产出③④的模型侧退出链）**：
  ...
  * **第三批待做**：qwen 的两个必抄细节（**approvalModeRevision 快照**：审批期间用户手动切档则批准作废；**enter/exit 视为执行边界**：同批后续工具调用跳过，留待下一轮模型观察新模式）、四端 mode 指示（CLI footer / web 状态条 / 移动端）、MockTransport 对等（设 plan 档时向事件流推 session.mode.changed）。
