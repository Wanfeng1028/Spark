...
> 进度（2026-09-05 第二次更新）：**R-A～R-H 全部完成 + 余量池全部闭合，阶段十七最终收官**（R-A/R-B 为 Qoder 会话；R-C～R-H 及余量池为 ZCode 会话 3df460e→尾批；doc/02 v3.80–v3.83；HEAD CI 绿）。拍板：Q-7 死文件**全部保留继续冻结**、Q-8 会话页控制器**纯逻辑状态机入 protocol**（均已实施/关闭）。余量池清账：R-E① 三批共 17 处收敛（Appearance/两 Dialog/SessionPage 回放三处判定保留）✅ / R-E⑦ Button outline+settingInputCls ✅ / R-E⑧ PageHeader ✅ / R-E⑨ useAsyncOp ✅ / R-E③ Composer 三浮层 ✅（菜单状态机判决不抽 hook）/ R-F③ routes 目录化六子插件 ✅ / R-G② app.tsx 二次拆分 ✅ / **R-D 门面残部判决不做**（mixin 需 any 违反 §2.4，透传即对外 API——boring code 判决）。
