...
| 19.38 | 🟡 诊断页（V2-14）**第一批：日志读取面已交付** | 新增 `packages/engine/src/logs.ts`（唯一读面，`Engine.logs()` 薄壳）：① **只读尾部字节窗口**（默认 512KB、上限 4MB——诊断日志会长到数百 MB，全量 readFile 会打爆常驻 server）；② 窗口起点落在文件中段 ⇒ 首行必是半行 ⇒ **丢掉不猜**（末行保留）；③ pino 数值级别与字符串级别都认（prettified 输出场景），其余字段原样保留（`pid/hostname/time/msg` 归信封不混入 fields）；④ **解析不出的行单独成条并标 `fields.unparsed`**——把"读不懂"伪装成"没有内容"是诊断页最不该有的行为；⑤ 文件不存在 = 空结果（首次运行前确实没日志，不是错误）。协议面 `LogEntryDto/LogsDto/LogsQuery` + `Transport.getLogs`，**三通道对等**（transport-node query 串 / sdk inprocess 直连 / web mock 合成日志但三道过滤走同一判据，path 明示是合成来源）；server `GET /api/logs` 走 `parseOr400`（非法 level/limit → 400，不在端上自判）；openapi 组件表补 `LogsDto/LogEntryDto`（缺登记则 `$ref` 自检直接红）。脱敏在写入侧已完成（logger 三类模式），读取侧不二次加工。**余第二批**：web「诊断」页（列表 + 级别过滤 + 子串匹配 + 前端导出当前视图）、mock 对等的 ui.keymap 合并（19.39 尾巴） | logs.test 8 例（缺文件为空 / 级别映射与信封 / "该级别及以上"语义 / match 比字段 / 半行丢弃 / 裸文本标 unparsed / limit 与 truncated / 字符串级别） | — |
