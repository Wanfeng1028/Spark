...
| 19.21 | 🟡 web 会话流小件批**第一批已交付** | client-commands 补 8 项中的 **7 项**：new（新建并跳转）/stats（跳使用统计页）/checkpoint（开检查点对话框）/tree（开会话树对话框）/effort（循环推理档 low→medium→high→provider 缺省）/fork（从最后一条消息 fork）/rollback（回滚到该事件检查点）——经 ui store 两个一次性信号（sessionDialogRequest/effortCycleSeq）驱动 SessionPage 既有对话框与 Composer 档位（零新端点）；**/help 刻意不映射**（web 无帮助面板——未实现端不渲染，禁假状态） | client-action-coverage.test 3 例（18 个 web client 命令 17 项映射 + /help 如实缺席/七项形状/键在封闭枚举内） | — |
| 19.22 | 🟡 Mock 对等修复批（§1.1 纪律）**第一批已交付** | 修两项真 bug + 三项行为对齐：① updateSettings 丢 agents/extensions patch（16.2/16.5 停用在 mock 下失效——补两段合并）；② createSession 丢 opts（显式 title/model 现覆盖演示脚本值）；③ updateMcpConfig 内存持久（保存后读回一致，此前恒初始值）；④ deleteSession 未知 id → E_NOT_FOUND（对等真实通道 404）；⑤ sendMessage 未知会话 → E_NOT_FOUND。**余项登记第二批**：listFs/listFsTree 静态、transcribe、arena 状态机、testModelProvider、getMcpConfig 掩码差异（mock 无 env 真值可掩码，如实保留） | mock-parity.test 6 例（两段不丢/opts 生效/mcp 读回一致/两个未知 id 拒执/已知会话照常） | — |
