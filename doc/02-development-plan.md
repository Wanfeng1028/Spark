...
| POST | /api/pair                 | `PairRedeemBody`                                     | `{ token }`（工单 9.1 移动端兑换口：6 位短码 → 长效 token；鉴权钩子豁免——自举；码无效/过期/重放 → 401 `E_PAIR`） |
| POST | /api/pair/code            | —                                                    | `PairCodeDto`（工单 9.1 桌面签发：60s 一次性短码 + QR 出示内容 `spark://pair?host=&port=&code=`；鉴权未启用 → 403 `E_PAIR_DISABLED`） |
...
| 配对鉴权未启用 | 403 `E_PAIR_DISABLED`（工单 9.1：无 devices.json 时调签发端点） |
