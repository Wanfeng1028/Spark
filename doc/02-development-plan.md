...
| 19.22 | 🟡 Mock 对等修复批（§1.1 纪律）**第一、二批已交付** | 第一批：① updateSettings 丢 agents/extensions patch；② createSession 丢 opts；③ updateMcpConfig 内存持久；④⑤ deleteSession/sendMessage 未知 id → E_NOT_FOUND。第二批：⑥ getMcpConfig 按 MCP_ENV_MASK 回显凭据（原实现明文回吐）；⑦ sendMessage 接 delivery（queue/steer 语义）与 expectedTurnId（E_TURN_MISMATCH）+ attachments；⑧ arena 三方法按会话归属（非属主 null / E_NOT_FOUND）+ listArenaHistory 接 limit（缺省 20 上限 100）。**余项登记第三批**：listFs/listFsTree 静态、transcribe、testModelProvider | mock-parity.test（掩码回显/delivery 不变量/arena 归属/limit 截断）＋ mock-transport.test 18 例签名同改 | — |
