...
| 19.21 | 🟡 web 会话流小件批**第一批已交付**（第二/三批见下） | client-commands 补 8 项中的 **7 项**：new（新建并跳转）/stats（跳使用统计页）/checkpoint（开检查点对话框）/tree（开会话树对话框）/effort（循环推理档 low→medium→high→provider 缺省）/fork（从最后一条消息 fork）/rollback（回滚到该事件检查点）——经 ui store 两个一次性信号（sessionDialogRequest/effortCycleSeq）驱动 SessionPage 既有对话框与 Composer 档位（零新端点）；**/help 刻意不映射**（web 无帮助面板——未实现端不渲染，禁假状态）。**第二批**：Composer 图片拖拽上传（与文件选择器同一条 uploadAttachment 通道，dragDepth 计数消进出抖动）。**第三批**：侧栏状态点归档灰档（`SessionStatusDot` archived 分支取 `--muted-foreground`，归档位 = `SessionMetaDto.archivedAt`）——`dotColor` 单源收敛仍需 mobile/miniapp ThemeTokens 补 `sparkMeta`（尾巴）。**余**：链接预览卡（V2-24）待拍板（SSRF 面 + 四包改动） | client-action-coverage.test 3 例（web client 命令映射除 /help 外全覆盖/七项形状/键在封闭枚举内）；SessionStatusDot.test 3 例（三态取色/归档压过状态色/缺省 false） | — |
