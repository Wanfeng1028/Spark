...
| 19.2 | ✅ computer-use macOS/Linux 执行体 + 电脑控制设置页 | engine `computer/macos.ts`（osascript System Events + screapture + pbpaste/pbcopy 全内置命令；辅助功能授权未放行如实 E_COMPUTER_EXEC；右/中键与 scroll 无内置面**如实报 E_COMPUTER_UNSUPPORTED 登记限制**）+ `computer/linux.ts`（xdotool/wmctrl/xclip/scrot，缺工具 E_COMPUTER_UNAVAILABLE 附安装提示；**Wayland 不支持如实拒绝**；app launch 直接 exec argv 无 shell）+ 工厂三平台路由 + web「电脑控制」页真落地（主开关 updateSettings 热档 / 八操作 computer:// 档位摘要只读——增删改归权限规则页 / 平台执行体三行说明）+ settings-pages computer→ready + ToolCard computer 族（Monitor 图标 + 截图渲染复用 artifacts 通道）+ flow-rows toolCategoryOf 电脑控制；CLI /computer 只读面板（命令基线 23→24 四处接线 + ComputerPanel 只读态）；mobile/miniapp 指示行（**DESIGN §13.J.2.11 条款先行**：只读单行卡三态 fail-soft + 开关单一来源在 web 设置中心；miniapp MiniRestClient.getSettings 单方法入 D21 体积预算） | ComputerSettingsPage.test 2 例 + render.test 基线 24 + round5 走查 15-settings-computer.png 全渲染；真机走查留用户（mac 授权 / Linux 工具 / 两端指示行） | 19.1 |
