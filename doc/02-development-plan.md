...
| 19.17 | 🟡 i18n 全量（翻案 Q-2，消解 V2-12）**第一批已交付** | ① 框架：protocol `i18n.ts`（LanguageSchema 封闭集 zh-CN/en + LANGUAGE_LABELS + detectLanguage 系统探测 + translate 点路径翻译 + 占位符替换 + **缺键回落中文再回落 key**——迁一半留一半时不报错不显示 key 名）；② 设置：spark.json `ui.language`（SettingsDto/SettingsUpdate 增 ui 段，热档切换即时生效，服务端单源）+ web I18nProvider（服务端 > localStorage > 系统探测）+「界面语言」行真控件（占位徽标摘除）；③ 迁移范围（第一批）：设置中心导航（组头/页名/返回）+ 通用动作词（shell.*/action.*/settings.* 三命名空间约 60 键）。**长尾页留第二批**（会话流/审批卡/错误文案/mobile/miniapp/CLI——既有 ui-copy/error-copy 单源整体接入翻译层，不拆散） | i18n.test 5 例（中英互译/缺键回落/占位符/SETTINGS_PAGE_KEY 覆盖/探测）；语言切换 round-trip 走 settings 路由既有断言 | — |
