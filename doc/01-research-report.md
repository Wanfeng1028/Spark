...
| v1.8 | 2026-08-26 | AI 编写：ZCode CLI · ox-alpha（model id `57d26d76-3d24-4c1c-95b3-88fcc03173f9/stealth/ox-alpha`）；发起：晚风（Wanfeng1028，D1 审计指令）                                                              | 派生文档登记：新增 doc/07-harness-audit.md（十九条学科×三态 + 工程六大类源码级审计，缺口 H01–H36）——本文 §10 参考体系为其"参考实现"列的唯一依据，阶段六~九路线图（doc/02 v3.0）以审计结论为输入；Python Worker 经对照本文参考体系判决"不做" |
| v1.9 | 2026-09-20 | AI 编写：ZCode CLI · step-5-preview（a6c5ff1d-d214-403d-aa90-3817d8cc9db2/step-5-preview）；发起与决策：晚风（Wanfeng1028，"其实可以抄的，新增加一条，参考项目 minimax code"指令） | §10 参考体系扩至 **11 项**：新增 #11 **MiniMax Code**（MiniMax-AI/minimax-code；MIT；终端 coding agent，约 1.5k★）——抄什么：headless 契约与结算退出纪律（packages/tui/src/headless：contract/events/output/settlement/exit-policy）、runtime→UI 事件桥与显示脱敏（packages/agent-core/src/event-bridge：bridge/converters/display-sanitize/runtime-warning）、pi 血统 turn runner 工程化（packages/agent-core/src/pi-turn-runner：llm-retry、tool-context-size、outbound-message-normalizer）、插件合同（packages/tui/src/plugin）。许可证经其仓内 LICENSE-STATUS.md 在线核验：第一方默认 MIT（root LICENSE + NOTICE 署名 MiniMax Code）可复用保留版权声明；例外 third_party/sandbox-runtime 为 Apache-2.0，Pi 派生的终端代码/模型目录/打包资产保留原声明。与 AGENTS v1.56、doc/02 v4.77、README v1.43 同批。**遗留待决**：§10 表 #7–#9 与"2026-08-23 增"两组历史重复行（v1.4–v1.7 并行会话追加所致）未修，修净与否待人类决策 |
| v2.0 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，"参考项目中加入 ZCode，质谱的开源版本，你去分析一下源码"指令） | **§7.1/§7.3 ZCode 状态翻案：闭源 → 已开源（Apache-2.0）**。zai-org/ZCode 2026-09-20 建仓约 6.8k★；TypeScript monorepo（apps/zcode-cli/packages/{core,adapters,contracts,bootstrap} + packages/{desktop,server}）。§10 参考体系扩至 **12 项**（#12 ZCode——三路并行源码调研进行中，报告完成后补详细借鉴条目）。**原判修正**：v1.0 时 ZCode 无官方开源仓库，§7.3 判"闭源不可参考"成立；2026-09-25 确认开源，原判不再适用。三路调研覆盖：① 架构/agent 循环/工具系统 ② 权限模型/协议/MCP/hooks ③ UI 渲染/亮点功能/可借鉴 TOP 5。**许可证核验**：Apache-2.0（root LICENSE），全部第一方代码可复用保留版权声明 || v2.0 | 2026-09-26 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，"参考项目中加入 ZCode，质谱的开源版本，你去分析一下源码"指令） | **§7.1/§7.3 ZCode 状态翻案：闭源 → 已开源（Apache-2.0）**。zai-org/ZCode 2026-09-20 建仓约 6.8k★；TypeScript monorepo（apps/zcode-cli/packages/{core,adapters,contracts,bootstrap} + packages/{desktop,server}）。§10 参考体系扩至 **12 项**（#12 ZCode——三路并行源码调研进行中，报告完成后补详细借鉴条目）。**原判修正**：v1.0 时 ZCode 无官方开源仓库，§7.3 判"闭源不可参考"成立；2026-09-25 确认开源，原判不再适用。三路调研覆盖：① 架构/agent 循环/工具系统 ② 权限模型/协议/MCP/hooks ③ UI 渲染/亮点功能/可借鉴 TOP 5。**许可证核验**：Apache-2.0（root LICENSE），全部第一方代码可复用保留版权声明 调研报告 → **doc/12-zcode-analysis.md**（三路合并：架构/循环/工具/上下文/权限/协议/MCP/hooks/亮点 TOP 10，file:line 级证据） |
...
文件头注释：_"Defines the protocol for a Codex session between a client and an agent. Uses a SQ (Submission Queue) / EQ (Event Queue) pattern."_
...
export type SessionEvent<T> = {
  [K in SessionEventType]: {
    type: K
    seq: number /* 恒等于 log.length */
    time: number
    data: SessionEventMap[K]
    ignorable?: true
  } & (K extends SurfaceEventType /* user/message|assistant/message|tool/result */
    ? { sourceEventSeqs?: number[]; surfaceOp?: 'append' | { op: 'replace'; start; end } }
    : object)
}[T]
```
...
读端 fail-closed：KNOWN_SESSION_EVENT_TYPES 生成词表（48 种，含 approval/_、hook/_、team/_、tool-workflow/_）——表外且无 ignorable **拒绝解释日志**。
...
- 前端 boot：client/modules 扫描 Loader entries 中 package.json **dsh.client** 字段（parseDshClient 校验 platform(必填)/inject/external/immediately；clientExportOf 解析 exports["./client"]）→ sha1 前 12 位 rev → WebBootGraph（orderByModuleGraph 拓扑+环检测）→ **DSH_BOOT** 注入；浏览器 lazy CJS 表：bundle 只 register factory（window.**ModuleLoader**.load({id,factory})），首次 require 才 materialize；引导两级 dsh-client-modules→dsh-client-runtime 先于 Vite shell。
...
### 7.3.1 Gemini CLI 借鉴点细化（文件级，2026-08-23 二轮调研）

1. **工具调度 + 审批总线**：`packages/core/src/scheduler/`（scheduler/state-manager/tool-executor/tool-modifier/policy/confirmation 六文件）——工具调用是状态机（Scheduled → Validating → Executing → Successful/Errored，类型在 scheduler/types.ts）；调度器不直接碰 UI，确认请求经 `confirmation-bus/message-bus.ts` 发布、CLI 侧订阅应答——**core 与 UI 彻底解耦，headless/交互/多 agent 共用同一调度器**。
2. **审批即策略（TOML Policy Engine）**：`policy/policy-engine.ts` + `policy/policies/*.toml`（内置 yolo/plan/read-only/write/non-interactive/sandbox-default/agents 七套）；规则带分层优先带 **Admin 5.x > User 4.x > Workspace 3.x > Extension 2.x > Default 1.x**；UI 的 "Always Allow"（4.95）、`--allowed-tools`（4.3）、MCP trust（4.2）都是动态注入的规则；YOLO=998 优先级 allow-all，但 ask_user=999 在任何模式下强制交互。
3. **单一事件流三端复用**：`utils/events.ts`（CoreEventEmitter）→ `agent/event-translator.ts` → `output/stream-json-formatter.ts`——一套事件同时驱动 TUI、stream-json 输出、telemetry。
4. **上下文压缩双层管道**：`context/{contextManager,chatCompressionService,toolDistillationService,toolOutputMaskingService,truncation}.ts`——区分"历史对话压缩"与"工具输出蒸馏"两层，配 pipeline/ 组织。
5. **LLM 客户端装饰器链**：`core/contentGenerator.ts`（logging/model-mapping/recording/fallback 中间件式包装）。

附加：`gemini-extension.json` manifest（context 文件+MCP server+斜杠命令+hooks+TOML 策略打包分发、从 GitHub 安装）值得作为扩展打包格式参考；GEMINI.md 分层（全局→项目→子目录，`@file` import 模块化）+ auto-memory；npm `@google/gemini-cli` 累计 717 版（nightly 日更）。治理注意（§7.3 表）：Google 正推动用户向闭源 Antigravity CLI 迁移（2026-08 社区争议），**pin 版本当快照用，不追新**。

### 7.3.2 Antigravity 可移植的四个交互范式（源码闭源，UX 层面参考）

①**Manager Surface 式多 agent 并行总览**——任务级 "mission control"，跨多 workspace 异步 spawn/编排/观察；②**Artifacts 一等公民 + 异步评论反馈**——task lists / implementation plans / walkthroughs（带截图的逐步指南）/ browser recordings，用户以 Google-doc 式评论 + 截图圈选批注，agent 后续消费反馈而**不中断执行**（异步反馈回路）；③**终端 + 浏览器双通道 Verify**（配套 Chrome 扩展接管浏览器做测试验证）；④**knowledge base 持久沉淀**复用。时间线：2025-11-20 与 Gemini 3 同发（public preview 免费，自带 Gemini 3 Pro/Claude Sonnet 4.5/GPT-OSS）；2026-08 已迭代至 2.0 并新增 `agy` CLI（与 GUI 共享同一 Core Agent Engine）；官方 GitHub org 仅 2 仓（antigravity-cli：NO-LICENSE 无源码，二进制经 install.sh 分发；antigravity-sdk-python：Apache-2.0 API 封装）。

### 7.3.3 OpenClaw Gateway 协议要点（细节补充）

- **Gateway 单守护进程**监听 `ws://127.0.0.1:18789`；线协议 = WS 文本帧 JSON，首帧必须 `connect`；**req/res/event 三类消息 + JSON Schema 校验 + 副作用方法（send/agent）强制幂等键**；事件含 agent/chat/presence/health/heartbeat/cron，`event:agent` 携带 runId 流式输出。
- 客户端拓扑：Control UI（Web）/CLI/TUI/macOS app 均为 WS 客户端；**Nodes**（macOS/iOS/Android/headless 设备）以 `role:node` 接入并声明 caps（camera._/screen.record/canvas._）；本地信任模型 = 共享密钥 + 设备配对（`openclaw pairing approve`）。
- **ACP 双向桥**：`/acp spawn` 可把 Claude Code/Cursor/Gemini CLI/OpenCode 等外部 coding harness 作为频道会话后端（`sessions_spawn({runtime:"acp"})`）；Gateway 也可暴露为 ACP server 供 IDE 连——证明"助理外壳 + coding agent 后端"解耦路线可行（Spark 后期挂接外部 agent 时直接借鉴）。
- 工程组织：23 个核心包中 `gateway-protocol/gateway-client/plugin-sdk/plugin-package-contract/session-url-contract/workboard-contract` **契约独立成包**（与我们 protocol 包同思路）；150+ extensions（30+ 消息频道、LLM 提供商、工具、记忆 lancedb/wiki/active-memory）+ ClawHub 插件市场；作者 Peter Steinberger（steipete），OpenClaw Foundation（非营利）持有版权；致谢 Mario Zechner（pi 作者——生态互认又一例）。
...
| 11  | MiniMax Code（2026-09-20 增）             | 终端 agent 的 headless 契约（输出/结算/退出码）、runtime→UI 事件桥与显示脱敏、pi 血统 turn runner 工程化（llm-retry/tool-context-size/出站消息归一化）、插件合同；第一方 MIT 可抄（保留版权声明） | MiniMax-AI/minimax-code：packages/tui/src/headless、packages/agent-core/src/event-bridge、packages/agent-core/src/pi-turn-runner、packages/tui/src/plugin |
...
_报告完（v1.7）。实施细节见 `02-development-plan.md`。_
