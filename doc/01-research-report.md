...
- 客户端拓扑：Control UI（Web）/CLI/TUI/macOS app 均为 WS 客户端；**Nodes**（macOS/iOS/Android/headless 设备）以 `role:node` 接入并声明 caps（camera.*/screen.record/canvas.*）；本地信任模型 = 共享密钥 + 设备配对（`openclaw pairing approve`）。
