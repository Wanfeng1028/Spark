...
| v1.53 | 2026-09-19 | AI 编写：ZCode CLI·GLM-5.3-Flash（`builtin:bigmodel-start-plan/GLM-5.3-Flash`）；发起：晚风（Wanfeng1028，"那你开始吧。等啥呢"开工指令） | **新增 D44 电脑控制执行体 = PowerShell 脚本桥零原生依赖（阶段十九 19.1；doc/02 v4.63 同批）**：平台分层端口 + Windows PowerShell 桥（CU_* 环境变量传参零注入面 + -EncodedCommand + 超时/中断 kill fail-closed）；主开关 engine.computerUseEnabled 缺省 false fail-closed 热档；八工具恒广告统一 computer.use 审批域；macOS/Linux 归 19.2。**编号注记**：原拟 D43 被并行会话（DSH 形态对齐）占用，顺延 D44。Windows 真机走查留用户。本机零验证，CI 裁决 |
...
>
> **2026-09-19 判决翻案总注记（晚风拍板，doc/08 §5D 立项依据④）**：阶段十九对以下**登记限制类**判决全量翻案立项——D15（沙箱网络隔离与 Windows OS 级沙箱后置）、D16（MCP 仅 stdio）、D19（CLI 单行输入）、D21（小程序分发中继记账）、D25（记忆/会话向量检索后置）、D42（arena 内存态）、16.9"无 LSP 下载器"、bash"不做常驻"、"always 恒用户级"审批作用域——各对应工单（19.3/19.4/19.5/19.6/19.7/19.8/19.9/19.10/19.25/19.29）落地时以迷你 ADR 修订本表对应条目；下表各条原文在其修订 ADR 合入前仍按原文执行。不变量级"不做"清单（显式 Planner 等）不在翻案范围。
...
决策：外部 MCP server 经 `~/.spark/mcp.json`（可选；version 1 + servers 表，stdio transport）声明。引擎构造时 `McpManager` 逐 server 连接（spawn + initialize + listTools，缺省 30s 墙钟上限——RT3-04 由 10s 上调（npx 冷启动普遍超 10s），per-server `connectTimeoutMs` 可覆盖、上限 600s）并把每个工具包成 `ToolDefinition` 注册进**同一 ToolRegistry**——命名 `mcp__<server>__<tool>`（register 重复名抛错兜底与内置冲突）、审批 `action=mcp.call` + `resource=<server>/<tool>`（默认 ask，permissions.json 三态规则照常生效）、`parallelizable=false`（外部进程副作用不透明，串行 barrier）、inputSchema 用 `z.fromJSONSchema`（materialize 的 toJSONSchema 往返已实证）。限界/溢写/事件纪律由管线免费复用——外部工具与内置四工具零差别路径。server 入口 `await engine.ready()` 后才 listen；shutdown 关闭全部子进程。
...
2. **SSRF 硬门**（voice/ssrf.ts，qwen 必抄项）：端点 fetch 前 DNS 解析全地址过 BlockList——IPv4 私网/环回/链路本地/CGNAT/保留段 + IPv6 ULA/组播 + **过渡段**（::ffff:0:0/96、64:ff9b::/96、64:ff9b:1::/48、2002::/16）；命中即 E_TRANSCRIBE_BLOCKED（403）。check-then-fetch 的 TOCTOU 与 qwen 同口径接受（v1 不做 IP pin）。**勘误（2026-09-12，修 CI 连红时）**：IPv4-mapped 段 `::ffff:0:0/96` 已撤——Node 把 IPv4 归一化为 `::ffff:<v4>` 存储/查询，整段入表 = 封禁全部 IPv4（公网端点全被误杀，voice.test 放行 8.8.8.8 等即红）；mapped 私网改由 BlockList 对 v4-mapped 地址的归一化命中既有 IPv4 私网段覆盖（voice.test 已加固点分 + 十六进制变体断言固化，CI 绿）。另 Node 24 起 IPv6 段的 addSubnet/check 须显式传 `'ipv6'` family，否则抛 Invalid socket address。
...
### D44 电脑控制执行体 = PowerShell 脚本桥零原生依赖（2026-09-19，阶段十九工单 19.1）
