...
- [ ] **新写测试用例仍是任务的一部分**（只是不在本机跑）：新增事件类型走 [new-event-type](./.agents/skills/new-event-type/SKILL.md) 全流程（27 种逐一单测）；新工具走 [new-tool](./.agents/skills/new-tool/SKILL.md)（四路径单测）。TypeScript strict 禁 `any`。
