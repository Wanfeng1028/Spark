...
- Up next: stage 19 batches A→J (19.26 is blocked on a real-machine recording; 19.34 distribution step is blocked on code-signing certificate purchase; 19.43 second batch needs the still-image assets downloaded; 19.44 is closed out — the OG image was re-exported and the slogan grep now runs as check 5 of the doc checker) → user-side walkthrough items ([doc/08](./doc/08-v2-roadmap.md)).
